...
for offset in range(0, 520, 20):
    url = f'https://book.douban.com/subject/35964293/comments/?start={offset}&limit=20&status=P&sort=score'
    print('scraping', url)
    ...
    for item in soup.select('.comment-item'):
        ...
        print(user, times)
        data.append({'User': user, 'Content': content, 'Time': times})
    time.sleep(1.5)

with open('output.csv', 'w', newline='', encoding='utf-8') as file:
    fieldnames = ['User', 'Content', 'Time']
